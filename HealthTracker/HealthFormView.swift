    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                Text("Health Profile")
                    .font(.custom("AvenirNext-Heavy", size: 48)) // Increased size and using Heavy weight for extra boldness
                    .foregroundStyle(
                        LinearGradient(
                ...
                Button(action: {
                    onSubmit(currentWeight, goalWeight, wantsClearSkin ? "Yes" : "No", String(dailyWaterIntake), String(exerciseHours), String(sleepHours))
                }) {
                    Text("Submit")
